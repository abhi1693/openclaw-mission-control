...
### Fixed
- **Review-only tasks stalling in `inbox`**: Two production stalls in two hours on 2026-05-09 (Track F `ab91d422` and Final acceptance `a8743f3a`) — Architect sign-off and QA-E2E acceptance tasks created with `review_packet_type='review_only'` landed in `status='inbox'` and could not advance. No actor could perform `inbox→review`: non-lead agents are blocked by the allowed-fields gate (rejects `review_packet_type`) and the `_AGENT_PATH_VALID_TRANSITIONS` gate ((inbox, review) not in the allowlist); leads were blocked by `_lead_apply_status` (status-gate forbids non-`review` current-state transitions) AND by the comment-required board flag; user-route was usable but only after manual `OperatorDecision` resolution. Each stall required manual operator unblock. Schema-level fix: `review_only` packets now start in `status='review'` directly on both create routes (`backend/app/api/tasks.py` user create + `backend/app/api/agent.py` agent-lead create), via the new free-function `normalize_review_only_initial_status()` in `backend/app/schemas/tasks.py`. The rule lives in a free function rather than a `TaskBase.model_validator` because `TaskRead` inherits `TaskBase` — a validator there would silently rewrite legacy stuck rows when the API serialized them via `TaskRead.model_validate(task, from_attributes=True)`, making the API misreport DB state.
- **PATCH recategorize-to-`review_only` did not auto-advance**: A PATCH that flipped `review_packet_type` to `review_only` without supplying a status left the task wedged in `inbox`. `update_task` in `backend/app/api/tasks.py` now flips `payload.status` to `review` BEFORE `model_dump(exclude_unset=True)` runs (placement matters — pre-rev1 Codex review caught a draft that mutated after the dump and silently no-op'd). Lead path gets a narrow exception in `_lead_apply_status` allowing `inbox→review` for `review_only` tasks. The exception drops the "comment provided" condition because `_validate_lead_update_request` at `tasks.py:4482` already rejects ANY lead PATCH carrying `comment is not None` — no point gating on a field the validator forbids upstream (plan rev 3 amendment).

### Added
- **`backend/scripts/backfill_review_only_inbox.py`**: One-shot backfill for legacy `review_only` tasks already wedged in `inbox`. Runs through `_apply_admin_task_rules` + `_finalize_updated_task` (same pattern as `normalize_board_delivery_contract.py`) so activity events fire and lead-assignment normalization runs — surgical UPDATEs were rejected as not auditable. Supports `--dry-run` and `--board-id` for scoped runs.

### Tests
- 16 tests across 2 new files: `backend/tests/test_review_only_inbox_bypass.py` (11: 4 creation + 5 PATCH + 1 lead recat-advance + 1 non-lead-agent reject regression guard) and `backend/tests/test_review_only_inbox_backfill.py` (5: advance / activity-event / dry-run / idempotency / board-scope).

### Out of scope
- Surfacing pending `OperatorDecision` rows in MC UI (separate plan), WhatsApp ping on `OperatorDecision` insert (separate plan), and a dedicated `ActivityEvent.actor_user_id` column (deferred — Task 6 attempt added it, rolled back to keep this shipset migration-free; today user-actor activity is recorded with `agent_id=None` — its absence is the user-actor signal, but a positive identifier requires a follow-up migration).

### Deployment
- No Alembic migration. Backend code changes pick up via `systemctl restart mc-backend` (per `feedback_create_all_at_startup` — MC has two units, this fix is backend-only).
...
### Fixed
- **Lead never woke when last unresolved dep cleared**: When task B `depends_on` task A and A transitioned to `done`, B's `is_blocked` flipped False at the read layer but `_reconcile_dependents_for_dependency_toggle` only wrote a `task.updated` activity event — no lead wake. Asymmetric with the blocker-resolve path which already wakes via `notify_lead_after_blocker_resolved`. Added sibling helper `notify_lead_after_dependency_cleared` and made the dep-cleared branch return the list of dependents that became fully unblocked; both production callsites now commit, then iterate the returned list and wake the lead per dependent. Symmetric with the post-commit wake on blocker resolve.
- **Pure-container umbrella tasks parked indefinitely**: Umbrellas linked to their executable work via `depends_on_task_ids` (no `parent_task_id` children) never qualified for `maybe_cascade_umbrella_close`. Added `maybe_retire_pure_container_umbrella` covering the `depends_on` graph edge, fired from the dep-clear hook, plus a board-wide `auto_retire_pure_container_umbrellas` sweep at `/lead/next-action` as the backstop for umbrellas that pre-date the hook or where the marker was posted after deps already terminated. Same retirement preconditions on both: `status=inbox`, never-executed, declared deps with all in `TERMINAL_STATUSES`, no open Blockers, no non-terminal `parent_task_id` children, **and** the explicit `UMBRELLA_RETIRED` marker comment.
- **Race window in pure-container umbrella retirement** (codex gpt-5.5/high review catch): The sweep ran inside the read-only `/lead/next-action` endpoint without any row lock, so a concurrent request that started/reopened/reassigned a candidate between SELECT and commit would be clobbered. Both `maybe_cascade_umbrella_close` and `maybe_retire_pure_container_umbrella` now use a conditional `UPDATE ... WHERE status='inbox' AND in_progress_at IS NULL AND previous_in_progress_at IS NULL` and skip the row when `rowcount=0`. The endpoint's OpenAPI `x-side-effects` annotation was updated to truthfully describe the cleanup pass.
- **Marker drop reverted** (codex gpt-5.5/high review catch): An earlier same-day commit relaxed both retirement helpers to fire on `never-executed + all-terminal children/deps` alone, dropping the `UMBRELLA_RETIRED` marker requirement. Codex flagged that the predicate matches both legitimate retired containers AND ordinary tasks waiting on a prerequisite dep — the marker is the only reliable discriminator. Marker requirement restored on both paths; the test fixture in `test_dependency_clear_wake.py` that had been patched with `previous_in_progress_at` to dodge the false-positive (the workaround that revealed the bug) was also dropped.
- **`@Supervisor` vs `@lead` citation inconsistency in verdict comments**: The `structured-review-verdict` write-time invariant rejected verdict comments that lacked `@Supervisor` (because the lead's gateway display name is `Supervisor`), but the older chat convention across all other comments was `@lead`. Reviewers had to use a different mention shape for verdicts than for chat. Validator now accepts either form (`@(?:Supervisor|lead)\b`); the four verdict-related skills (`architect-review-verdict`, `qa-validation-verdict`, `reviewer-recheck`, `structured-review-verdict`) now show `@lead` in canonical templates and document both as equivalent.

### Changed
- **`_reconcile_dependents_for_dependency_toggle` return type**: Was `None`, now `list[Task]` of dependents that became fully unblocked on this dep clear. All callers updated to commit, then iterate, then wake.
- **`_send_lead_wake` parameter sprawl removed**: The shared helper no longer takes `suppress_log_label`. Each public wrapper (`notify_lead_after_blocker_resolved`, `notify_lead_after_dependency_cleared`) handles its own `try/except` + `logger.warning`.
- **`_try_unblock_or_retire_dependent` extracted**: The five sequential `continue` branches in the dep-cleared loop of `_reconcile_dependents_for_dependency_toggle` collapsed into a single `Task | None`-returning helper.

### Added
- **`docs/bugs/2026-05-05-gateway-misclassifies-clean-ws-1000-as-failed.md`**: Gateway bug report for transmission to OpenClaw maintainers. Symptom: ACP children that complete cleanly are recorded in `task_runs.sqlite` with `status=failed` and `error="WebSocket closed 1000"` whenever the upstream WS closes with RFC 6455 normal closure, even when the on-disk artifacts (commit, evidence files, screenshots) confirm the run succeeded. Concrete reproducer is the Phase 3 / E.05 incident: child `eb9454aa` ran prismjs syntax-highlighting impl + 4-locale Playwright spec + screenshots, committed `73e3adb` at 23:41:55Z, but the gateway recorded `ended_at=23:36:19Z` and `status=failed` ~6 minutes early. Doc covers root-cause hypothesis (WS-1000 vs `subagent-status: done` layer mismatch + lagging `ended_at`), suggested fix, and forensic queries.
