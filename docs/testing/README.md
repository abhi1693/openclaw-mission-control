...
Notes:
- Some tests may require a running Postgres (see root `compose.yml` / `backend/README.md`).
...
> If `npm run cypress` doesn’t exist yet, add it to `frontend/package.json` (or run `npx cypress open`).
