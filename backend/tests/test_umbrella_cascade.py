...
async def test_cascade_skips_parent_without_umbrella_retired_marker(
    ...
    """Safety: a parent that was never-picked-up but doesn't carry the
    explicit ``UMBRELLA_RETIRED`` comment must NOT auto-cancel just
    because its children happen to be done. Without the marker, the
    "never executed + all-terminal children" predicate matches both
    legitimate retired containers AND ordinary tasks waiting on their
