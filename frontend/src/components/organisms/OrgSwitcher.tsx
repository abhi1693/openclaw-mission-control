  ...
  return (
    <div className="relative">
      <Select value={orgValue} onValueChange={handleOrgChange}>
        <SelectTrigger className="h-9 w-[220px] rounded-md border-[color:var(--border)] bg-[color:var(--surface)] px-3 text-sm font-medium text-strong shadow-none focus:ring-2 focus:ring-[color:var(--accent-soft)] focus:ring-offset-0">
          <span className="flex items-center gap-2">
            <Building2 className="h-4 w-4 text-muted" />
            <SelectValue placeholder="Select organization" />
          </span>
        </SelectTrigger>
        <SelectContent className="min-w-[220px] rounded-md border-[color:var(--border)] p-1 shadow-xl bg-[color:var(--surface)]">
          <div className="px-3 pb-2 pt-2 text-[10px] font-semibold uppercase tracking-wide text-muted">
            Org switcher
          </div>
          {orgs.length ? (
            orgs.map((org) => (
              <SelectItem
                key={org.id}
                value={org.id}
                className="rounded-md py-2 pl-7 pr-3 text-sm text-strong data-[state=checked]:bg-[color:var(--surface-muted)] data-[state=checked]:text-strong focus:bg-[color:var(--surface-muted)]"
              >
                {org.name}
              </SelectItem>
            ))
          ...
          <SelectItem
            value="__create__"
            className="rounded-md py-2 pl-3 pr-3 text-sm font-medium text-muted hover:text-strong focus:bg-[color:var(--surface-muted)] [&>span:first-child]:hidden"
          >
            <span className="flex items-center gap-2">
      ...
      {orgError && !createOpen ? (
        <p className="absolute left-0 top-full mt-1 text-xs text-[color:var(--danger)]">
          {orgError}
        </p>
      ) : null}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent aria-label="Create organization">
          ...
          <div className="mt-4 space-y-2">
            ...
            />
            {orgError ? (
              <p className="text-sm text-[color:var(--danger)]">{orgError}</p>
            ) : null}
          </div>
