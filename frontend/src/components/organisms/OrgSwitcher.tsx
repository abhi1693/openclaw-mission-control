  ...
  return (
    <div className="relative">
      <Select value={orgValue} onValueChange={handleOrgChange}>
        <SelectTrigger className="h-9 w-[220px] rounded-md border-[var(--border)] bg-[var(--surface)] px-3 text-sm font-medium text-[var(--text)] shadow-none focus:ring-2 focus:ring-[var(--accent)]/30 focus:ring-offset-0">
        ...
        <SelectContent className="min-w-[220px] rounded-md border-[var(--border)] p-1 shadow-xl">
          ...
          {orgs.length ? (
            orgs.map((org) => (
              <SelectItem
                key={org.id}
                value={org.id}
                className="rounded-md py-2 pl-7 pr-3 text-sm text-[var(--text-muted)] data-[state=checked]:bg-[var(--surface-muted)] data-[state=checked]:text-[var(--text)] focus:bg-[var(--surface-strong)]"
              >
                {org.name}
              </SelectItem>
            ))
          ...
          <SelectItem
            value="__create__"
            className="rounded-md py-2 pl-3 pr-3 text-sm font-medium text-[var(--text-muted)] hover:text-[var(--text)] focus:bg-[var(--surface-strong)] [&>span:first-child]:hidden"
          >
            <span className="flex items-center gap-2">
      ...
      {orgError && !createOpen ? (
        <p className="absolute left-0 top-full mt-1 text-xs text-[var(--danger)]">
          {orgError}
        </p>
      ) : null}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent aria-label="Create organization">
          ...
          <div className="mt-4 space-y-2">
            ...
            />
            {orgError ? (
              <p className="text-sm text-[var(--danger)]">{orgError}</p>
            ) : null}
          </div>
